...
```bash
cd ~/dan-hebrew && git pull && ./install.sh
```

הסקריפט בטוח להרצה חוזרת: הוא משנה רק מה שעוד לא במקום, ומפעיל מחדש את Hammerspoon.
...
צילום מסך שנשמר כקובץ (`Cmd+Shift+3` / `Cmd+Shift+4` / `Cmd+Shift+5`) נכנס אוטומטית ל-Recent, בלשונית Images. הקובץ עצמו נשאר במקומו. כל צילום נכנס פעם אחת בלבד — גם אחרי טעינה מחדש.

כדי שהצילום ייכנס מיד, הכלי מכבה את התצוגה המקדימה הקטנה שמופיעה בפינת המסך אחרי צילום (בלעדיה macOS כותב את הקובץ רק אחרי ~5 שניות). להחזרה: `Cmd+Shift+5` → Options → Show Floating Thumbnail.
